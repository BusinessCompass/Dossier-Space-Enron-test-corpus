...
- v1.4: Complete rebuild with JSON-RPC protocol, FastMCP 1.29.0 compatible, clean separation
...
def create_mcp_server(custody_server: CustodyServer, ledger: LedgerWriter) -> Server:
    ...
    async def call_tool(name: str, arguments: dict):
        ...
        try:
            if name == "search_messages":
                result = custody_server.search_messages(
                    custodian=arguments.get("custodian"),
                    keyword=arguments.get("keyword"),
                    date_contains=arguments.get("date_contains")
                )
            elif name == "list_folder":
                result = custody_server.list_folder(custodian=arguments.get("custodian"))
            elif name == "read_file":
                result = custody_server.read_file(filepath=arguments.get("filepath"))
            elif name == "get_headers":
                result = custody_server.get_headers(filepath=arguments.get("filepath"))
            elif name == "hash_document":
                result = custody_server.hash_document(filepath=arguments.get("filepath"))
            elif name == "verify_hash":
                result = custody_server.verify_hash(
...
async def main():
    ...
    server = create_mcp_server(custody, ledger)
    
    logger.info(f"MCP Custody Server v1.4 initialized and ready")
